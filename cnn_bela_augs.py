...
classes_select = [      #5x5 quand grosses images (abies, picea)/ higher epoch count ~200
                       "abb_pic_mix",
                    # "abies_b",
                    # "acer_mix",
                    # "acer_r",
...
else: path_folder_image = "/home/paleolab/Documents/python/CNN_BELA/pollen_dataset_2/level_0"
...
checkpoint_no ="256_resolution_test"
...
train_images, test_images, train_labels, test_labels, train_filenames, \
    test_filenames = train_test_split(images, cls, filenames, test_size=0.20, random_state=choose_random)
    # 0.15 -> 0.2
...
if simple_model == True :
  ...
  model = tf.keras.models.Sequential()
  model.add(tf.keras.layers.Conv2D(32, (3,3), input_shape=(resolution, resolution, 1), activation='relu', padding='same'))
  model.add(tf.keras.layers.Conv2D(32, (3,3), activation='relu', padding='same'))
  model.add(tf.keras.layers.MaxPooling2D())
  model.add(tf.keras.layers.Conv2D(64, (3,3), activation='relu', padding='same'))
  model.add(tf.keras.layers.Conv2D(64, (3,3), activation='relu', padding='same'))
