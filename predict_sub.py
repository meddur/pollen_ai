...
checkpoint_abb_pic = "abb_pic_softmax"
checkpoint_acer = "acer_softmax_test"
checkpoint_alnus = 'alnus_softmax_normal_depth'
checkpoint_pinus = "pinus_softmax"
checkpoint_tricolp = "tricolp_softmax_deep"
checkpoint_tripor = "transfer_tripor_small_deep"
checkpoint_path = "checkpoints/"+checkpoint_no+"/cp-{epoch:04d}.ckpt"

data_path = os.getcwd()+"/images_pre_260_276" #Images to be classified
...
classes_list = (          # List of all classes
...
print("DELETE THIS (FIRST CLASSIFICATION")
...
testo = pd.read_csv(data_path_post+"/testo.csv")
compilation = pd.DataFrame(compilation)

cols = testo.columns
compilation.rename(columns=dict(zip(compilation.columns, cols)), inplace=True)

from pandas.testing import assert_frame_equal
assert_frame_equal(compilation, testo, check_dtype=False)
